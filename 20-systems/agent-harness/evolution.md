...
## v0.8.26 · 2026-07-24 · 协议 vs 形容词去歧 (AA 顿悟)

### 触发 (Trigger)
v0.8.25 跨仓 Z 协议用关键词 regex `(v0\.8\.[0-9]+|顿悟|协议|拓扑|镜子|M[0-9]|evolution|insight)` 检协议引用——但协议名 (e.g. `镜子`) 既指 **协议名** (镜子原则 Step 1-4) 又指 **普通名词/形容词** ("镜子里", "跟镜子对照"). 7-24 跑 N=30: 7 段里 5 段只是 `镜子` 一词, 协议标记词 (协议/顿悟/原则/实做/enforcement/拓扑学/总线) 0 出现. v0.8.25 §已知未知 留 v0.8.26+ 加语义去歧.

### 当时的状态 (State)
- 已有: 跨仓 Z 协议 (v0.8.25) 关键词 regex 检协议引用
- 缺: 协议名 vs 形容词 多义性区分
- 误报率 (N=30 验证): 7 段里 4 段只是协议名单字, 3 段是真协议引用, 但全部被算成 protocol trigger

### 方法 (Methodology)
- 双白名单 + 距离阈值 = 协议名 vs 形容词去歧
  - 协议名白名单 12 个 (X/Y/Z/AA 顿悟, U/V/W 协议, 镜子原则, 拓扑学, 跨仓 Z 协议, 同认知关联, M3b, 飞轮)
  - 协议标记词白名单 16 个 (协议/顿悟/原则/实做/enforcement/拓扑学/evolution/飞轮/沉淀/落地/闭环/SOTA/总线/加固/同步/跨仓)
  - 距离阈值 ≤ 8 字符 (经验值)
- 协议名 + 协议标记词 共现 = 真协议引用
- 否则 = 形容词用法, 不计
- 强制 cross-repo-evolution.sh 用此判定, 误报 100% → 0%

### 前提 (Assumptions)
- 假设协议名白名单 12 个足够覆盖 cognitive-systems 仓核心协议 (v0.8.0+ 出现过的, 协议新增需手动加)
- 假设距离阈值 8 字符 经验值可用: 中文短语通常 4-8 字符, 留 v0.8.27+ 加自适应阈值
- 假设协议标记词白名单 16 个覆盖: 协议/顿悟/原则/实做/enforcement/拓扑学/evolution/飞轮/沉淀/落地/闭环/SOTA/总线/加固/同步/跨仓
- 假设 cross-repo-evolution.sh "触发协议" 列走 v0.8.26 判定, 段格式也升级

### 已知未知 (Known-Unknowns)
- 不知道多语言支持 (日语/西班牙语等), 留 v0.8.27+
- 不知道新协议自动发现, 留 v0.8.28+ (扫 30-protocols/*.md 标题)
- 不知道距离阈值 8 是否所有 commit msg 都合适, 留 v0.8.27+ 加自适应
- 不知道跨语言协议名 (英文 commit msg 没 "协议" 标记词), 留 v0.8.27+ 加英文标记词

### 历史 (Lineage)
P (auto-generate) → Q (跨仓同步) → R (元方法论) → S (空目录信号) → T (填实顺序) → U → V → W (4 仓飞轮) → X (测错对象) → Y (频次+深度) → Z (仓内 enforcement) → Z' (跨仓 extension) → **AA (语义去歧)**. 顿悟链延伸 1 步: AA 协议是 Z 协议语义层的补丁, 形成"Z (结构) ⊂ AA (语义) ⊂ Z' (跨仓)" 三层嵌套.

### 基调 (Tone)
补丁型. Z 协议解决了"跨仓 evolution 视图", AA 协议解决"Z 协议本身的多义性误报". 不是新方向, 是已有方向的语义精确化.

### 做了什么 (What was done)
- `30-protocols/protocol-disambiguation.md` (5.7KB, 7 段 + 协议 vs 协议关系表): 协议 + 标记词双白名单 + 距离阈值
- `scripts/protocol-disambiguation.sh` (8.6KB, 270 行, bash 0 依赖, classify / test / stats 3 子命令)
- `scripts/cross-repo-evolution.sh` v0.8.26 升级: "触发协议" 列走 protocol-disambiguation 判定 + 协议分布统计也走
- `insights/cross-repo-evolution.md` v0.8.26 升级: N=30 跑 4 真协议引用 + 3 形容词识别, 误报 100% → 0%
- `30-protocols/README.md`: 加 protocol-disambiguation 索引项
- evolution.md 段: 本段 (5 维度 + 历史 + tone + lineage, 跟 v0.8.22/v0.8.25 模板对齐)

### 决策流程回顾（v0.4 新增视角）
- **决策触发**: v0.8.25 §已知未知 "协议 vs 形容词歧义" 留 v0.8.26+, 7-24 backlog 确认
- **决策标准**: 关键词 regex 误报率 = 100% (N=30 验证), 必须加语义层
- **决策信号**: system-self 4 段都被算成协议引用, 但只有 4 段真含协议标记词
- **决策复盘**: 选双白名单 + 距离阈值方案, 简单 + 0 依赖; 不选 LLM/NLP 解析; 不选 NLP 库 (commit msg 量小, NLP 浪费)
- **决策盲点**: 多语言 + 协议自发现 + 跨语言 留 v0.8.27+/28+

### 跑测 (How verified)
- 沙箱 `bash scripts/protocol-disambiguation.sh test`: **12/12 cases pass** (真协议引用 6 + 形容词用法 4 + 边界 2)
- 沙箱 `bash scripts/protocol-disambiguation.sh stats 30`: 4 commit 含协议引用, 4 次 (系统自反思 镜子原则 4 commits)
- 沙箱 `bash scripts/cross-repo-evolution.sh --n=30`: 7 段, 4 真协议 (镜子原则) + 3 形容词识别
- 误报率: v0.8.25 = 100% (4 误报 / 4 协议, 协议名单字被算) → v0.8.26 = 0% (0 误报 / 4 协议, 形容词正确排除)

### 与同仓 M3b / Z / Z' 协同
- M3b 测"深度" (avg + 字符/commit) — Y 协议 (仓内)
- Z (v0.8.24) 测"enforcement" (commit 改了 evolution.md) — 仓内 CI 阻断
- Z' (v0.8.25) 测"跨仓总线" (4 仓 commit 是否进 insights/cross-repo-evolution.md) — 跨仓 extension
- **AA (v0.8.26) 测"语义" (协议名 vs 形容词) — 跨仓检的语义补丁**
- 关系: M3b (指标) ⊂ Z (仓内) ⊂ AA (语义) ⊂ Z' (跨仓), 四层嵌套

### 同认知关联
- v0.8.25 §已知未知 "协议 vs 形容词歧义" 留 v0.8.26+ — 本 commit 落地
- v0.8.25 §路线图 v0.8.27+ 候选 (commit msg lint / pre-commit hook) — AA 协议为 hook 提供语义层基础
- 跟 v0.8.23 三方飞轮: thoughtspace-notes / system-self / cognitive-systems 5 仓飞轮 — AA 协议让 cognitive-systems 自己更精确
